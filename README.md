...
| 键 | 说明 | 默认 |
|---|---|---|
| `show_windows` | 是否弹出同屏大面板（顶部传感器/导航状态区 + 下方左/右大恒和红外实时回传） | `true` |
| `fullscreen` | 面板全屏展示：窗口无边框占满整屏；顶部状态区固定保留，剩余高度按屏幕比例分给三路图像，图像等比居中留边 | `true` |
...
- 面板为全屏窗口（`fullscreen`，默认开）：屏幕分辨率经 xrandr 自动探测（失败时用 `screen_width`/`screen_height` 参数兜底）。顶部状态区高度 212px：左侧为 `600×200` 传感器框，中间为同尺寸导航通信框；左/右/红外三张实时回传图保持原有左/中/右顺序和横向位置，在状态区下方绘制，因此不被状态框遮挡。总图右下角显示实际刷新率。
- 传感器数据全部打印：T/RH/CO2 三值 + 温湿度/CO2 逐字段有效位（TH=ok/FAIL, CO2=ok/FAIL）+ 数据年龄 age；数据陈旧超过 3s 按离线显示。
- 导航协议实时块（顶部状态区中间框，与传感器框等宽等高）：`NAV->VIS 0x.. VIS->NAV 0x..` + 状态词——`WAIT CMD`=等待指令、`CAPTURING`=取图中、`DONE (wait nav 0x00)`=取图完成等导航确认；颜色区分（绿=空闲/黄=取图中/橙=完成待确认）。
