...
void CoreNode::DrawNavOverlay(cv::Mat & canvas)
{
    ...
    // has_* 标志是关键: "从没收到消息"与"收到 0x00"必须区分 —— 字节初值
    // 恰好都是 0, 不看标志就会把"导航没发"误当成"导航在发 0x00(行车)"。
    uint8_t cmd = 0;
    uint8_t st = 0;
    bool has_cmd = false;
    bool has_st = false;
    ...
    double cap_elapsed = 0.0;
    {
        ...
        has_cmd = has_nav_cmd_;
        has_st = has_nav_status_;
        const auto now = std::chrono::steady_clock::now();
    ...
    const std::string st_age_txt = has_st ?
        cv::format("%.1fs ago", st_age) : "no msg yet";
    const std::string cmd_byte_txt = has_cmd ?
